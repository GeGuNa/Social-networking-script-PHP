...
$KkkK2_id = filter($_GET['post']);
	

$post = $pdo->fetchOne("SELECT * FROM `gallery_komm` WHERE `id` = ?",[$KkkK2_id]);
...
$kzftch322 = $pdo->queryFetchColumn("SELECT COUNT(*) FROM `user_post_reaction` WHERE `post_id` = ? and `where` = 'foto'",[$post['id']]);
...
<?
$query = $pdo->query("SELECT * FROM `user_post_reaction` WHERE `post_id` = ? and `where` = 'foto' ORDER BY `when` DESC LIMIT $ttlq, $set[p_str]", [$post['id']]); ?>
...
<?
$qq->setPage("/foto/reacts.php?", "post=$post[id]");
...
<div class="Lfoot">
 <a href="/foto/foto.php?id=<?=$post['id_foto'];?>">Back</a>
</div>
