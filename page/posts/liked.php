...
if ($pdo->queryFetchColumn("SELECT COUNT(*) FROM `notes` WHERE `id` = ?", [$kqp123idd1]) == 0)
{
...
<div class="pzqwe2_ttlqw2z">Reactions</div>
...
$q213 = $pdo->query("SELECT * FROM `notes_like` WHERE `id_notes` = '".$note['id']."' ORDER BY `id` DESC LIMIT $ttlq, $set[p_str]");



while ($data = $q213->fetch())
{
...
<div class="dflexAcenter">
	...
	<div class="date"><?echo when($data['when']);?></div>
</div>
...
<div class='czFTr21 brdrtop'>
	<a href="/page/posts/?id=<?=$note['id'];?>">Back</a>
</div>
...
$qq->setPage("/page/posts/liked.php?", "id=$note[id]");
