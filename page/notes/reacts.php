...
$k1k2p3 = filter($_GET['post']);


$post = $pdo->fetchOne("SELECT * FROM `notes_komm` WHERE `id` = ?", [$k1k2p3]);
...
$kzftch322=$pdo->queryFetchColumn("SELECT COUNT(*) FROM `user_post_reaction` WHERE `post_id` = '".$post['id']."' and `where` = 'blogs'");
...
<?
$query = $pdo->query("SELECT * FROM `user_post_reaction` WHERE `post_id` = '".$post['id']."' and `where` = 'blogs' ORDER BY `when` DESC LIMIT $ttlq, $set[p_str]"); ?>
...
<?
$qq->setPage("/page/notes/reacts.php?", "post=$post[id]");
...
<div class="Lfoot">
 <a href="/page/notes/list.php?id=<?=$post['id_notes'];?>">Back</a>
</div>
