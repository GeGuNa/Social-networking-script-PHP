...
<div class="kAppPagePhtMusicHeading">
		...
		<div class="lmkmnaingq331">Bookmarked music</div>
</div>
...
$kzftch322 = $pdo->query("SELECT * FROM `post_music_bookmark` WHERE `whom` = ?",[$user['id']])->rowCount();
...
$qfetch = $pdo->query("SELECT * FROM `post_music_bookmark` WHERE `whom` = ? ORDER BY `bid` DESC LIMIT $ttlq, $set[p_str]", [$user['id']]);
...
while ($post3 = $qfetch->fetch())
{

$post = $pdo->fetchOne("select * from `post_music` where `uid` = ?", [$post3['mid']]);
