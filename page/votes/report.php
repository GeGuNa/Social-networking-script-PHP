...
$rpostid = $pdo->fetchOne("SELECT * FROM `poll_comments` WHERE `id` = ?", [$pid]);
...
$qbaduser = new user($rpostid['user']);
...
$directlink = my_esc("/page/votes/poll.php?id=$rpostid[poll]");
...
header("Location: poll.php?id=".$rpostid['poll']);
exit;
}
...
<div class='ftrwithoutborder'>
 <a href='poll.php?id=<?=$rpostid['poll'];?>'>უკან</a>
</div>	
