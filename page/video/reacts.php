...
$kzi1p = filter($_GET['post']);

$post = $pdo->fetchOne("SELECT * FROM `video_komm` WHERE `id` = ?", [$kzi1p]);
...
$kzftch322 = $pdo->queryFetchColumn("SELECT COUNT(*) FROM `user_post_reaction` WHERE `post_id` = '".$post['id']."' and `where` = 'video'");
...
<?
$query = $pdo->query("SELECT * FROM `user_post_reaction` WHERE `post_id` = '".$post['id']."' and `where` = 'video' ORDER BY `when` DESC LIMIT $ttlq, $set[p_str]"); ?>
...
<?
$qq->setPage("/page/video/reacts.php?", "post=$post[id]");
...
<div class="Lfoot">
 <a href="/page/video/video.php?id=<?=$post['id_video'];?>">Back</a>
</div>
